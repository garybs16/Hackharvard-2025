...
// MARK: - PDF Viewer Window (macOS / visionOS) -> now shows ReaderView
...
struct PDFViewerWindow: View {
    ...
    var body: some View {
        Group {
            ...
            } else if let doc = pdfManager.pdfDocument {
                // Interactive PDF + paragraph tap + TTS + voice controls
...
// MARK: - iOS PDF Viewer Sheet -> now shows ReaderView
...
// MARK: - (Legacy) PDF Page View (kept; grid not used by viewer now but safe to keep)
...
struct PDFPageView: View {
    ...
    var body: some View {
        ...
        .contextMenu {
            Button("View Full Size") { /* Future: Open full-size view */ }
            Button("Extract Text") { /* Future: Extract text from this page */ }
        }
    }
    
    @ViewBuilder
    private var ZstackImage: some View {
        ZStack {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ForEach(lines.indices, id: \.self) { i in
                        Text(lines[i])
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: 16)
                                    ...
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 16)
                                            .stroke(
                                                i == highlightIndex ? .yellow.opacity(0.3) : .gray.opacity(0.1),
                            ...
                            .onTapGesture {
                                withAnimation(.easeInOut(duration: 0.3)) { highlightIndex = i }
                            }
                    }
                }
